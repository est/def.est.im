...
2. `entries` 非空，每条含非空 `pos` / `definition` / `translation`。
3. `pos` 在枚举白名单内。
4. `example_en` 与 `example_zh` 成对（同有同无）。
5. `collocations` 若存在，必须是字符串列表（block 序列解析结果），且非空。
6. 解析失败或任一规则不过 → 附带解析器报错原文重试。
